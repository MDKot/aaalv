...
async function scrapeLivBeach() {
  console.log('Launching browser...');
  const browser = await puppeteer.launch({ headless: true });
  const page = await browser.newPage();

  console.log('Navigating to LIV Beach...');
  await page.goto('https://www.livnightclub.com/lasvegas/liv-beach', {
  ...
  const scrapedEvents: ScrapedEvent[] = events.map((event) => {
    ...
    const slug = slugify(`${event.artist}-liv-beach-${dateStr}`, { lower: true, strict: true });
    ...
    return {
      ...
      offer: 'Complimentary Admission until 2PM | Buy Tickets Below',
...
scrapeLivBeach().catch(console.error);
